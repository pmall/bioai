...
- **The antibody design bottleneck is collapsing.** Germinal achieves epitope-targeted de novo antibody design with nanomolar affinities using only 43-101 experimental tests per antigen. ProteinGuide enables on-the-fly conditioning of any pretrained generative model without retraining. Aureka's OpenDDE achieves 76% DockQ on antibody-antigen tasks. The infrastructure for programmable, rapid antibody design is maturing: computational design → low-n experimental validation → therapeutic candidates. (Observation: Germinal Jun 2026, ProteinGuide Jul 2026, Aureka Aug 2026)
